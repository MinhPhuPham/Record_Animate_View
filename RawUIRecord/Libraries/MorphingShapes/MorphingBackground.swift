...
struct MorphingBackground: View {
    ...
    func update(_ proxy: GeometryProxy){
        ...
        for i in 0..<number {
            ...
            if maxX < minX {
                circles[i] = MorphingCircle(size: proxy.size.width / 2, morphingRange: proxy.size.width / 20, color: circles[i].color, points: circles[i].points, duration:  circles[i].duration, secting: circles[i].secting)
            ...
            if maxY < minY {
                circles[i] = MorphingCircle(size: proxy.size.width / 3, morphingRange: proxy.size.width / 30, color: circles[i].color, points: circles[i].points, duration:  circles[i].duration, secting: circles[i].secting)
    ...
    func createCircles() {
        ...
        for _ in 0..<number {
            ...
            circles.append(MorphingCircle(size: circleSize, morphingRange: circleSize / 10.0, color: .red))
